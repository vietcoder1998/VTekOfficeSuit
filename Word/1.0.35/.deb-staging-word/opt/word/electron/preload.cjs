/**
 * packages/Word/electron/preload.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK Word Document Studio — Electron Preload Bridge
 *
...
function loadElectronModule() {
  try {
    ...
    console.warn(
      "[Word Preload] Electron module not available in current environment:",
...
const electronBridgeApi = {
  ...
  minimizeWindow: () => {
    try {
      ...
      console.error("[Word Preload Error] minimizeWindow failed:", minimizeError);
  ...
  maximizeWindow: () => {
    try {
      ...
      console.error("[Word Preload Error] maximizeWindow failed:", maximizeError);
      return Promise.resolve(false);
    }
  },
  ...
  closeWindow: () => {
    try {
      ...
      console.error("[Word Preload Error] closeWindow failed:", closeError);
      return Promise.resolve(false);
    }
  },
  ...
  isMaximized: () => {
    try {
      ...
      console.error("[Word Preload Error] isMaximized failed:", checkMaximizedError);
      return Promise.resolve(false);
    }
  },
  ...
  onWindowStateChange: (callback) => {
    ...
    const listener = (_event, isMaximizedState) => {
      try {
        ...
        console.error("[Word Preload Error] onWindowStateChange listener error:", listenerError);
      }
    };
    ...
    return () => {
      try {
        ...
        console.error("[Word Preload Error] Failed to remove listener:", removeError);
      }
    };
  },
  ...
  openFileDialog: (options = {}) => {
    try {
      ...
      console.error("[Word Preload Error] openFileDialog failed:", openDialogError);
      return Promise.resolve({ canceled: true });
    }
  },
  ...
  saveFileDialog: (options = {}) => {
    try {
      ...
      console.error("[Word Preload Error] saveFileDialog failed:", saveDialogError);
      return Promise.resolve({ canceled: true });
    }
  },
  ...
  readFile: (targetFilePath) => {
    try {
      ...
      console.error("[Word Preload Error] readFile failed:", readFileError);
  ...
  writeFile: (targetFilePath, fileContentText) => {
    try {
      ...
      console.error("[Word Preload Error] writeFile failed:", writeFileError);
  ...
  onExternalFileOpen: (callback) => {
    ...
    const listener = (_event, openedFilePath) => {
      try {
        ...
        console.error("[Word Preload Error] onExternalFileOpen listener error:", listenerError);
      }
    };
...
if (contextBridge && typeof contextBridge.exposeInMainWorld === "function") {
  try {
    ...
    console.error("[Word Preload Error] Failed to expose electronAPI via contextBridge:", exposeError);
