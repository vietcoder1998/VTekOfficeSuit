...
async function startDevelopmentWorkflow() {
  ...
  if (!isAlreadyRunning) {
    ...
    const isStandaloneMode =
      process.env.NODE_ENV === "production" || fileSystem.existsSync(serverJsPath);
    const isStandaloneProduction = isStandaloneMode;

    if (isStandaloneProduction && fileSystem.existsSync(serverJsPath)) {
      console.log(`[Word Runner] Dual-starting Word Next.js standalone server on port ${TARGET_PORT}...`);
      nextServerProcess = spawn(process.execPath, [serverJsPath], {
        cwd: appDirectory,
        stdio: "inherit",
        env: {
          ...process.env,
      ...
      console.log(`[Word Runner] Dual-starting Word Next.js dev server on port ${TARGET_PORT}...`);
    ...
    nextServerProcess.on("error", (spawnError) => {
      console.error("[Word Runner] Failed to spawn Next.js server:", spawnError);
    });
  } else {
    console.log(`[Word Runner] Next.js server is already active on port ${TARGET_PORT}`);
  }
  ...
  const cleanupSignalHandler = cleanupChildProcesses;
