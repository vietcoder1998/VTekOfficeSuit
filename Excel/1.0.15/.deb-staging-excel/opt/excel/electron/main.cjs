/**
 * packages/Excel/electron/main.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK Excel Spreadsheet Studio — Electron Main Process
 *
 * Provides desktop window lifecycle management, titlebar controls,
 * native file dialogs, and IPC channels for 2-TEK Excel Spreadsheet Studio.
...
function loadElectronModule() {
  try {
    if (typeof global !== "undefined" && global.__excel_electron_mock__) {
      return global.__excel_electron_mock__;
    }
    ...
    console.warn(
      "[Excel Electron] Electron module unavailable in current environment:",
 ...
 */
function displayElectronError(title = "2-TEK Excel Spreadsheet Studio Error", error = null, detail = "") {
  try {
    const errorMessage =
      error && error.message
        ? error.message
    ...
    console.error(
      `[Excel Electron Error] ${title}: ${errorMessage}`,
    ...
    const activeDialog =
      ...
      dialog ||
      (typeof global !== "undefined" && global.__excel_electron_dialog__);
    ...
    console.error("[Excel Electron Error] Failed to display native error dialog:", displayException);
    return false;
  }
}
...
function configureAppMetadata() {
  try {
    if (app && typeof app.setName === "function") {
      app.setName("2-tek-excel");
    }
    if (app && typeof app.setAppUserModelId === "function") {
      app.setAppUserModelId("com.vtek.excel");
    }
  } catch (metadataError) {
...
/**
 * Resolve target port (default: 3026)
 */
function resolvePortFromEnv() {
  try {
    ...
    console.error("[Excel Electron Error] Failed resolving port from env:", portError);
  }
...
function createMainWindow() {
  try {
    ...
    const windowOptions = {
      ...
      minWidth: 800,
      minHeight: 600,
      title: "2-TEK Excel Spreadsheet Studio",
    ...
    mainWindow.loadURL(APP_URL).catch((loadError) => {
      console.warn(`[Excel Electron] Retrying connection to ${APP_URL}:`, loadError.message);
      setTimeout(() => {
        if (mainWindow && !mainWindow.isDestroyed()) {
