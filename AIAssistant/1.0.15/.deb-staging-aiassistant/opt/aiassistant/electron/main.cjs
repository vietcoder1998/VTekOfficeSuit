/**
 * packages/AIAssistant/electron/main.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK Autonomous AI Assistant — Electron Main Process
 *
 * Provides desktop window lifecycle management, titlebar controls,
 * native file dialogs, and IPC channels for 2-TEK Autonomous AI Assistant.
...
function loadElectronModule() {
  try {
    if (typeof global !== "undefined" && global.__ai_assistant_electron_mock__) {
      return global.__ai_assistant_electron_mock__;
    }
    ...
    console.warn(
      "[AIAssistant Electron] Electron module unavailable in current environment:",
 ...
 */
function displayElectronError(title = "2-TEK Autonomous AI Assistant Error", error = null, detail = "") {
  try {
    const errorMessage =
      error && error.message
        ? error.message
    ...
    console.error(
      `[AIAssistant Electron Error] ${title}: ${errorMessage}`,
    ...
    const activeDialog =
      ...
      dialog ||
      (typeof global !== "undefined" && global.__ai_assistant_electron_dialog__);
    ...
    console.error("[AIAssistant Electron Error] Failed to display native error dialog:", displayException);
    return false;
  }
}
...
function configureAppMetadata() {
  try {
    if (app && typeof app.setName === "function") {
      app.setName("2-tek-ai-assistant");
    }
    if (app && typeof app.setAppUserModelId === "function") {
      app.setAppUserModelId("com.vtek.ai-assistant");
    }
  } catch (metadataError) {
...
/**
 * Resolve target port (default: 3021)
 */
function resolvePortFromEnv() {
  try {
    ...
    console.error("[AIAssistant Electron Error] Failed resolving port from env:", portError);
  }
...
function createMainWindow() {
  try {
    ...
    const windowOptions = {
      ...
      minWidth: 800,
      minHeight: 600,
      title: "2-TEK Autonomous AI Assistant",
    ...
    mainWindow.loadURL(APP_URL).catch((loadError) => {
      console.warn(`[AIAssistant Electron] Retrying connection to ${APP_URL}:`, loadError.message);
      setTimeout(() => {
        if (mainWindow && !mainWindow.isDestroyed()) {
