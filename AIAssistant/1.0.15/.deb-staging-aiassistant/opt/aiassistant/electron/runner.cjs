/**
 * packages/AIAssistant/electron/runner.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK Autonomous AI Assistant — Electron Development Orchestrator
 *
 * Checks/starts the Next.js dev server on port 3021, waits for HTTP readiness,
...
function resolvePortFromEnv() {
  try {
    if (typeof process !== "undefined" && process.env) {
      ...
      if (process.env.AI_ASSISTANT_PORT) {
        const parsedAppPort = parseInt(process.env.AI_ASSISTANT_PORT, 10);
    ...
    for (const candidateEnvironmentFilePath of candidateEnvironmentFiles) {
      if (fileSystem.existsSync(candidateEnvironmentFilePath)) {
        ...
        for (const singleLine of fileLines) {
          ...
          if (variableKey === "PORT" || variableKey === "AI_ASSISTANT_PORT") {
    ...
    console.error("[AIAssistant Runner Error] Failed resolving port from env:", resolvePortError);
  }
...
function checkServerReady(targetUrl = DEV_SERVER_URL) {
  try {
    ...
    console.error("[AIAssistant Runner Error] checkServerReady unexpected failure:", checkError);
    return Promise.resolve(false);
  }
}

async function waitForServer(targetUrl = DEV_SERVER_URL) {
  console.log(`[AIAssistant Runner] Waiting for dev server at ${targetUrl}...`);
  for (let attemptNumber = 1; attemptNumber <= MAXIMUM_POLL_ATTEMPTS; attemptNumber += 1) {
    ...
    if (isReady) {
      console.log(`[AIAssistant Runner] Dev server is ready at ${targetUrl} (attempt ${attemptNumber})`);
      return true;
    }
    await new Promise((resolve) => setTimeout(resolve, POLL_INTERVAL_MILLISECONDS));
  }
  console.warn(`[AIAssistant Runner] Server did not become ready after ${MAXIMUM_POLL_ATTEMPTS} attempts, launching anyway.`);
  return false;
}
...
async function startDevelopmentWorkflow() {
  ...
  if (!isAlreadyRunning) {
    console.log(`[AIAssistant Runner] Starting AIAssistant Next.js server on port ${TARGET_PORT}...`);
    ...
    nextServerProcess = spawn(npmCommand, ["run", "dev"], {
      ...
      env: {
        ...
        AI_ASSISTANT_PORT: String(TARGET_PORT),
      },
    });

    nextServerProcess.on("error", (spawnError) => {
      console.error("[AIAssistant Runner] Failed to spawn Next.js dev server:", spawnError);
    });
  } else {
    console.log(`[AIAssistant Runner] Dev server is already active on port ${TARGET_PORT}`);
  }
  ...
  console.log(`[AIAssistant Runner] Launching Electron with ${electronExecutableCommand}...`);
  ...
  electronChildProcess.on("exit", (exitCode) => {
    console.log(`[AIAssistant Runner] Electron exited with code ${exitCode}`);
...
if (require.main === module) {
  startDevelopmentWorkflow().catch((runnerError) => {
    console.error("[AIAssistant Runner Error] Execution failed:", runnerError);
    process.exit(1);
  });
}
