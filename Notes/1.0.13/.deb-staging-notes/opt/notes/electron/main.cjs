/**
 * packages/Notes/electron/main.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK Notes & Memo Studio — Electron Main Process
 *
 * Provides desktop window lifecycle management, titlebar controls,
 * native file dialogs, and IPC channels for 2-TEK Notes & Memo Studio.
...
function loadElectronModule() {
  try {
    if (typeof global !== "undefined" && global.__notes_electron_mock__) {
      return global.__notes_electron_mock__;
    }
    ...
    console.warn(
      "[Notes Electron] Electron module unavailable in current environment:",
 ...
 */
function displayElectronError(title = "2-TEK Notes & Memo Studio Error", error = null, detail = "") {
  try {
    const errorMessage =
      error && error.message
        ? error.message
    ...
    console.error(
      `[Notes Electron Error] ${title}: ${errorMessage}`,
    ...
    const activeDialog =
      ...
      dialog ||
      (typeof global !== "undefined" && global.__notes_electron_dialog__);
    ...
    console.error("[Notes Electron Error] Failed to display native error dialog:", displayException);
    return false;
  }
}
...
function configureAppMetadata() {
  try {
    if (app && typeof app.setName === "function") {
      app.setName("2-tek-notes");
    }
    if (app && typeof app.setAppUserModelId === "function") {
      app.setAppUserModelId("com.vtek.notes");
    }
  } catch (metadataError) {
...
/**
 * Resolve target port (default: 3030)
 */
function resolvePortFromEnv() {
  try {
    ...
    console.error("[Notes Electron Error] Failed resolving port from env:", portError);
  }
...
function createMainWindow() {
  try {
    ...
    mainWindow.loadURL(APP_URL).catch((loadError) => {
      console.warn(`[Notes Electron] Retrying connection to ${APP_URL}:`, loadError.message);
      setTimeout(() => {
        if (mainWindow && !mainWindow.isDestroyed()) {
