/**
 * packages/Notes/electron/runner.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK Notes & Memo Studio — Electron Development Orchestrator
 *
 * Checks/starts the Next.js dev server on port 3030, waits for HTTP readiness,
...
function resolvePortFromEnv() {
  try {
    if (typeof process !== "undefined" && process.env) {
      ...
      if (process.env.NOTES_PORT) {
        const parsedAppPort = parseInt(process.env.NOTES_PORT, 10);
    ...
    for (const candidateEnvironmentFilePath of candidateEnvironmentFiles) {
      if (fileSystem.existsSync(candidateEnvironmentFilePath)) {
        ...
        for (const singleLine of fileLines) {
          ...
          if (variableKey === "PORT" || variableKey === "NOTES_PORT") {
    ...
    console.error("[Notes Runner Error] Failed resolving port from env:", resolvePortError);
  }
...
function checkServerReady(targetUrl = DEV_SERVER_URL) {
  try {
    ...
    console.error("[Notes Runner Error] checkServerReady unexpected failure:", checkError);
    return Promise.resolve(false);
  }
}

async function waitForServer(targetUrl = DEV_SERVER_URL) {
  console.log(`[Notes Runner] Waiting for dev server at ${targetUrl}...`);
  for (let attemptNumber = 1; attemptNumber <= MAXIMUM_POLL_ATTEMPTS; attemptNumber += 1) {
    ...
    if (isReady) {
      console.log(`[Notes Runner] Dev server is ready at ${targetUrl} (attempt ${attemptNumber})`);
      return true;
    }
    await new Promise((resolve) => setTimeout(resolve, POLL_INTERVAL_MILLISECONDS));
  }
  console.warn(`[Notes Runner] Server did not become ready after ${MAXIMUM_POLL_ATTEMPTS} attempts, launching anyway.`);
  return false;
}
...
async function startDevelopmentWorkflow() {
  ...
  if (!isAlreadyRunning) {
    console.log(`[Notes Runner] Starting Notes Next.js server on port ${TARGET_PORT}...`);
    ...
    nextServerProcess = spawn(npmCommand, ["run", "dev"], {
      ...
      env: {
        ...
        NOTES_PORT: String(TARGET_PORT),
      },
    });

    nextServerProcess.on("error", (spawnError) => {
      console.error("[Notes Runner] Failed to spawn Next.js dev server:", spawnError);
    });
  } else {
    console.log(`[Notes Runner] Dev server is already active on port ${TARGET_PORT}`);
  }
  ...
  console.log(`[Notes Runner] Launching Electron with ${electronExecutableCommand}...`);
  ...
  electronChildProcess.on("exit", (exitCode) => {
    console.log(`[Notes Runner] Electron exited with code ${exitCode}`);
...
if (require.main === module) {
  startDevelopmentWorkflow().catch((runnerError) => {
    console.error("[Notes Runner Error] Execution failed:", runnerError);
    process.exit(1);
  });
}
