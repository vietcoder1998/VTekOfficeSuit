/**
 * packages/SuperChat/electron/preload.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK SuperChat Client — Electron Preload Bridge
 *
...
function loadElectronModule() {
  try {
    ...
    console.warn(
      "[SuperChat Preload] Electron module not available in current environment:",
...
const electronBridgeApi = {
  ...
  minimizeWindow: () => {
    try {
      ...
      console.error("[SuperChat Preload Error] minimizeWindow failed:", minimizeError);
  ...
  maximizeWindow: () => {
    try {
      ...
      console.error("[SuperChat Preload Error] maximizeWindow failed:", maximizeError);
      return Promise.resolve(false);
    }
  },
  ...
  closeWindow: () => {
    try {
      ...
      console.error("[SuperChat Preload Error] closeWindow failed:", closeError);
      return Promise.resolve(false);
    }
  },
  ...
  isMaximized: () => {
    try {
      ...
      console.error("[SuperChat Preload Error] isMaximized failed:", checkMaximizedError);
      return Promise.resolve(false);
    }
  },
  ...
  onWindowStateChange: (callback) => {
    ...
    const listener = (_event, isMaximizedState) => {
      try {
        ...
        console.error("[SuperChat Preload Error] onWindowStateChange listener error:", listenerError);
      }
    };
    ...
    return () => {
      try {
        ...
        console.error("[SuperChat Preload Error] Failed to remove listener:", removeError);
      }
    };
  },
  ...
  openFileDialog: (options = {}) => {
    try {
      ...
      console.error("[SuperChat Preload Error] openFileDialog failed:", openDialogError);
      return Promise.resolve({ canceled: true });
    }
  },
  ...
  saveFileDialog: (options = {}) => {
    try {
      ...
      console.error("[SuperChat Preload Error] saveFileDialog failed:", saveDialogError);
      return Promise.resolve({ canceled: true });
    }
  },
  ...
  readFile: (targetFilePath) => {
    try {
      ...
      console.error("[SuperChat Preload Error] readFile failed:", readFileError);
  ...
  writeFile: (targetFilePath, fileContentText) => {
    try {
      ...
      console.error("[SuperChat Preload Error] writeFile failed:", writeFileError);
  ...
  onExternalFileOpen: (callback) => {
    ...
    const listener = (_event, openedFilePath) => {
      try {
        ...
        console.error("[SuperChat Preload Error] onExternalFileOpen listener error:", listenerError);
      }
    };
...
if (contextBridge && typeof contextBridge.exposeInMainWorld === "function") {
  try {
    ...
    console.error("[SuperChat Preload Error] Failed to expose electronAPI via contextBridge:", exposeError);
