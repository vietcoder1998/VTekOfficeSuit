/**
 * packages/SuperChat/electron/main.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK SuperChat Client — Electron Main Process
 *
 * Provides desktop window lifecycle management, titlebar controls,
 * native file dialogs, and IPC channels for 2-TEK SuperChat Client.
...
function loadElectronModule() {
  try {
    if (typeof global !== "undefined" && global.__super_chat_electron_mock__) {
      return global.__super_chat_electron_mock__;
    }
    ...
    console.warn(
      "[SuperChat Electron] Electron module unavailable in current environment:",
 ...
 */
function displayElectronError(title = "2-TEK SuperChat Client Error", error = null, detail = "") {
  try {
    const errorMessage =
      error && error.message
        ? error.message
    ...
    console.error(
      `[SuperChat Electron Error] ${title}: ${errorMessage}`,
    ...
    const activeDialog =
      ...
      dialog ||
      (typeof global !== "undefined" && global.__super_chat_electron_dialog__);
    ...
    console.error("[SuperChat Electron Error] Failed to display native error dialog:", displayException);
    return false;
  }
}
...
function configureAppMetadata() {
  try {
    if (app && typeof app.setName === "function") {
      app.setName("2-tek-super-chat");
    }
    if (app && typeof app.setAppUserModelId === "function") {
      app.setAppUserModelId("com.vtek.super-chat");
    }
  } catch (metadataError) {
...
/**
 * Resolve target port (default: 3016)
 */
function resolvePortFromEnv() {
  try {
    ...
    console.error("[SuperChat Electron Error] Failed resolving port from env:", portError);
  }
...
function createMainWindow() {
  try {
    ...
    mainWindow.loadURL(APP_URL).catch((loadError) => {
      console.warn(`[SuperChat Electron] Retrying connection to ${APP_URL}:`, loadError.message);
      setTimeout(() => {
        if (mainWindow && !mainWindow.isDestroyed()) {
