/**
 * packages/SuperChat/electron/runner.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK SuperChat Client — Electron Development Orchestrator
 *
 * Checks/starts the Next.js dev server on port 3016, waits for HTTP readiness,
...
function resolvePortFromEnv() {
  try {
    if (typeof process !== "undefined" && process.env) {
      ...
      if (process.env.SUPERCHAT_PORT) {
        const parsedAppPort = parseInt(process.env.SUPERCHAT_PORT, 10);
    ...
    for (const candidateEnvironmentFilePath of candidateEnvironmentFiles) {
      if (fileSystem.existsSync(candidateEnvironmentFilePath)) {
        ...
        for (const singleLine of fileLines) {
          ...
          if (variableKey === "PORT" || variableKey === "SUPERCHAT_PORT") {
    ...
    console.error("[SuperChat Runner Error] Failed resolving port from env:", resolvePortError);
  }
...
function checkServerReady(targetUrl = DEV_SERVER_URL) {
  try {
    ...
    console.error("[SuperChat Runner Error] checkServerReady unexpected failure:", checkError);
    return Promise.resolve(false);
  }
}

async function waitForServer(targetUrl = DEV_SERVER_URL) {
  console.log(`[SuperChat Runner] Waiting for dev server at ${targetUrl}...`);
  for (let attemptNumber = 1; attemptNumber <= MAXIMUM_POLL_ATTEMPTS; attemptNumber += 1) {
    ...
    if (isReady) {
      console.log(`[SuperChat Runner] Dev server is ready at ${targetUrl} (attempt ${attemptNumber})`);
      return true;
    }
    await new Promise((resolve) => setTimeout(resolve, POLL_INTERVAL_MILLISECONDS));
  }
  console.warn(`[SuperChat Runner] Server did not become ready after ${MAXIMUM_POLL_ATTEMPTS} attempts, launching anyway.`);
  return false;
}
...
async function startDevelopmentWorkflow() {
  ...
  if (!isAlreadyRunning) {
    console.log(`[SuperChat Runner] Starting SuperChat Next.js server on port ${TARGET_PORT}...`);
    ...
    nextServerProcess = spawn(npmCommand, ["run", "dev"], {
      ...
      env: {
        ...
        SUPERCHAT_PORT: String(TARGET_PORT),
      },
    });

    nextServerProcess.on("error", (spawnError) => {
      console.error("[SuperChat Runner] Failed to spawn Next.js dev server:", spawnError);
    });
  } else {
    console.log(`[SuperChat Runner] Dev server is already active on port ${TARGET_PORT}`);
  }
  ...
  console.log(`[SuperChat Runner] Launching Electron with ${electronExecutableCommand}...`);
  ...
  electronChildProcess.on("exit", (exitCode) => {
    console.log(`[SuperChat Runner] Electron exited with code ${exitCode}`);
...
if (require.main === module) {
  startDevelopmentWorkflow().catch((runnerError) => {
    console.error("[SuperChat Runner Error] Execution failed:", runnerError);
    process.exit(1);
  });
}
