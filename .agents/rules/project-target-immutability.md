...
1. **Hub (`Packages/Hub`)**:
   - Canonical Target: **Run web app Next.js with standalone mode** (Chạy ứng dụng web Next.js với chế độ standalone). Launch Next.js apps from builder when drop drag zip file to Hubs as launcher apps running from folders in standalone mode.
   - Scope: Desktop application launcher, process manager, port allocator, Next.js standalone runner (`server.js`, `.next/standalone`), apps storage management (`APPS_STORAGE_PATH`), desktop packaging (`.deb`, `.exe`).
...
7. **OfficePack (`Packages/OfficePack`)**:
   - Canonical Target: **Unified Office Suite engines, Word, Excel, Presentation, PDF, Forms, Notes, AI Chat, and backend Server Gateway**
   - Scope: Document parsers, spreadsheets, presentation decks, PDF viewing, AI assistant, server gRPC/HTTP gateway, local-db persistence.
   - Prohibitions: NEVER turn OfficePack into a general code IDE canvas or standalone marketing website.
