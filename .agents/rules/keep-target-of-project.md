# Mandatory Rule: Follow and Keep Target of Project via PROJECT_TARGET.md — Zero Target Drift (Rule 69)

<!-- Compatibility: QUY TẮC BẮT BUỘC: TUÂN THỦ VÀ DUY TRÌ MỤC TIÊU DỰ ÁN QUA PROJECT_TARGET.MD — KHÔNG ĐỔI MỤC TIÊU VÀ CÁCH LY CHỨC NĂNG (KEEP TARGET OF PROJECT RULE — ZERO TOLERANCE) -->
<!-- Test Alias: RULE 69 / KEEP TARGET OF PROJECT RULE — ZERO TOLERANCE -->

> **ZERO TOLERANCE**:
> Across the entire 2-TEK ecosystem, the root workspace, all packages in `Packages/*` (`AIAssistant`, `Antivirus`, `Bots`, `Cloud`, `CompanyWeb`, `DeviceFarms`, `Excel`, `Forms`, `HealthCheck`, `Hub`, `Iconbuilder`, `Lowcode`, `Notes`, `Pdf`, `Presentation`, `Server`, `Settings`, `Shared`, `Studio`, `SuperChat`, `SystemAdmin`, `Tasks`, `Word`), and all developers and AI Coding Agents:
> 1. **MANDATORY 100%: ROOT AND ALL PACKAGES MUST HAVE PROJECT_TARGET.MD**:
>    - The root workspace **MUST** maintain `PROJECT_TARGET.md` defining the macro mission of the 2-TEK platform and the canonical target matrix of all packages.
...
>      1. **Read `PROJECT_TARGET.md`** (at root and `Packages/{packageName}/PROJECT_TARGET.md`) to confirm alignment with the canonical target.
>      2. **Read `README.md`** to verify local operational commands and routes (Rule 64).
>      3. **Read `.standards/`** (`structure.md` and `designs.md`) to adhere to layout and theme rules (Rule 68).
...
>    - ❌ **FORBIDDEN** to bypass reading `PROJECT_TARGET.md` before initiating code modifications.
...
| **CompanyWeb** | Corporate public web presence & customer portal | NEVER embed lowcode AST compilation engines or bot daemons. |
| **SystemAdmin** | Administrative operations, RBAC & telemetry | NEVER bypass Edge middleware authentication; login first. |
...
[Developer or AI Agent receives task for Package X]
  ...
  Inspect Canonical Target & Anti-Goals for Package X
    ...
    & Packages/X/.standards/     Redirect task to the
...
- [ ] `PROJECT_TARGET.md` exists at workspace root.
