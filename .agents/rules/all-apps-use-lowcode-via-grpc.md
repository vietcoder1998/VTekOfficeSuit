...
>    - Every application in `Packages/` (including `AiAssistant`, `LowcodeStudio`, `Office`, `Word`, `Excel`, `Presentation`, `Pdf`, `Forms`, `Notes`, `SuperChat`, `Cloud`, `DeviceFarms`, `Bots`, `Server`, etc.) that requires Lowcode capabilities, engines, document operations, AI tools, or project manipulation **MUST** communicate exclusively by calling gRPC through package-local gRPC clients (e.g. `@/lib/grpc` or `@/app/api/_grpc`). `@Shared` must no longer be used for gRPC calls.
