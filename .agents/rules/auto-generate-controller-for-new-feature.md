...
>    - When a new feature is logged in `.agents/features/*.md`, the system and AI Agent **MUST** automatically generate a corresponding controller extending `BaseController`.
...
2. **Automated Feature-to-API Lifecycle**:
   - Ensures every feature logged in `.agents/features/*.md` immediately receives an active API endpoint for testing, UI binding, and MCP integration.
