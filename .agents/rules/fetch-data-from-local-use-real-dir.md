...
>    - **Default Workspace Discovery Fallback**: If no user folder is selected yet, populate local storage exclusively from real files that exist within the current workspace directory (`./`, `Lowcode/.agents/standards/Office/`, etc.) with authentic sizes and paths.
