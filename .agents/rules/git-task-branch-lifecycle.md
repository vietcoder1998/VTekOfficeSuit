...
1. Register MCP Standard First (`actions.json`, `schemas/`, `templates/`, `standard-specification.md`):
   - *Note*: If the task is a bug fix, file/folder move or rename, or component styling/props modification, **DO NOT implement to MCP** (strictly exempt, skip Step 1); otherwise (new feature/tool/service), MCP registration is mandatory.
