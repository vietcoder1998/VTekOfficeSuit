...
>    - **Zero Border & Default White Background (Rule 71)**: Tab strip containers and toolbar wrappers must not use borders (`border: none;`, `border-bottom: none;`) and must render with pure white background (`#ffffff` / `var(--surface, #ffffff)`) in the default theme.
...
>    - Governed automatically by `LcOfficeToolbarGroupsEngine` (`Lowcode/sources/office/lc-office-toolbar-groups-engine.ts`) and registered in `Lowcode/.agents/standards/Office/standard-specification.md` under Section 22 and `.agents/rules/toolbar-no-border-white-background.md` (Rule 71).
...
6. ❌ **FORBIDDEN** to use borders or non-white background on Office toolbars in the default theme (Rule 71).
