...
> **ZERO TOLERANCE**:
> Across all applications and packages in the 2-TEK workspace (`Packages/Bots`, `Packages/Office`, `Packages/LowcodeStudio`, `Packages/Lowcode`, `Packages/OfficePack`, `Packages/SuperChat`, `Packages/Cloud`, `Packages/Tasks`, `Packages/DeviceFarms`, `Packages/Hub`, `Packages/Settings`, etc.):
...
> 5. **ZERO BORDER & WHITE BACKGROUND IN DEFAULT THEME (RULE 71 ALIGNMENT)**:
>    - All toolbars, ribbons, and control bars must have no borders (`border: none;`, `border-bottom: none;`) and their background color in the default theme must be pure white (`#ffffff` / `var(--surface, #ffffff)` / `background-color: #ffffff;`).
> 6. **GOVERNANCE & AUDIT**:
>    - Governed by `.agents/rules/toolbar-only-display-icons.md` (Rule 65) and `.agents/rules/toolbar-no-border-white-background.md` (Rule 71), registered in workspace standards (`Packages/Shared/standards/types.ts`).
