...
4. **Task Iteration & Queue Finishing (Rule 67)**: For any queued tasks, list all queue items into `.agents/features/` first, and begin finishing tasks at the last queue item; upon task completion, mark as done (`- [x]`) and review upcoming feature items in `.agents/features/`.

## 2. Feature Creation Workflow

- **Step 000: Queue & Feature Logging First (Rule 67)**:
  When receiving any input queue, batch tasks, or multi-item prompt, developers and AI agents MUST list ALL items from the queue into `.agents/features/{dd-mm-yyyy}.md` (prioritizing current date) as uncompleted checkbox tasks (`- [ ] ...`) before executing any task. Only at the last queue item (once all queue items are recorded in `.agents/features/`), start executing and finishing the tasks.
...
- **Step 0: Example File & Standards/Schema First (Rule 49)**:
  For any new feature, generator, or data model, provide or create at least one concrete example file first (in `.example/`, template fixture, or test suite), update core file handling logic (parser, reader, writer, validator), and update file standards and schemas in `.agents/standards/` (and `Lowcode/.agents/standards/MCP/schemas/`). Feature generation is only permitted once file standards and schemas are established.
- **Step 1: MCP Standard First (Exempt for Bug Fixes, Move File/Folder & Components Change)**:
  For new capabilities, define tool actions, schemas, and templates in `Lowcode/.agents/standards/MCP/` (`actions.json`, `schemas/`, `templates/`, `standard-specification.md`) before implementation.
  > **MCP EXEMPTION RULE**:
...
- **Step 3: UI Implementation with .agents/standards & Theme**:
  When creating or modifying `.tsx` files (pages, components, layouts, dialogs), strictly follow `Lowcode/.agents/standards/IDE/default-settings.json`, utilize system theme tokens, and compose with Base components defined in `Packages/Shared/components/{projectNames}/bases` (mapped via `@/components/bases` or `@Shared/components/{projectName}/bases`).
...
- Across all Next.js projects (`OfficePack`, `SuperChat`, `Clouds`, `Office`, `DeviceFarms`, `Bots`, `LowcodeStudio`):
...
- Every package under `Packages/{name}/` must have a `.agents/standards/` directory containing `structure.md` and `designs.md`.
- Developers and AI agents must read `Packages/{name}/.agents/standards/` alongside `README.md` (Rule 64) before updating or generating files.
- All code, directory layouts, and UI components must strictly conform to the package's local `structure.md` and `designs.md`.

## 23. Path & Directory Configuration via .env Only (Rule 72)
...
- All source code, background scripts, and engines must resolve paths dynamically via `process.env` or `.env` files with generic fallbacks (`os.homedir()`, `process.cwd()`).
- Strictly prohibited to hardcode personal user paths (`/home/viettd`, `/home/tranduyviet`, `/home/user`, `/Users/...`) in source code or scripts.
