...
| **bot** | **Control feature in Production** | • Production Mission Control Bot (port 3012)<br>• Telegram topic/supergroup & Box Chat bi-directional sync<br>• Antigravity AI agent orchestration & queued task runner<br>• Feature monitoring & notification system (`.features/`)<br>• Docker build gating, release verification & deployment control | ❌ NEVER turn bot into a document suite editor, client web portal, or visual lowcode canvas designer. Bot is the Production Governor. |
| **.lowcode** | **LowcodePrograming** | • Core Lowcode programming language, runtime & engine<br>• Standard AST schemas & definitions (`.lowcode/.standards/`)<br>• MCP tool actions & contracts (`actions.json`, `schemas/`)<br>• Headless state machines, validators & cross-platform code generators | ❌ NEVER treat `.lowcode` as casual UI code, throwaway scripts, or office-specific file logic. It is the programming foundation. |
...
1. **`devloper` (Coder)**:
   ...
   - Enforce Rule 67: When receiving a queue or batch of tasks, list all items into `.features/` first, then in the last queue, start finishing the tasks.
...
3. **`git-specialist` (Git Checker)**:
   ...
   - Enforce Rule 67 & Rule 1: Queue & Feature logging in `.features/{dd-mm-yyyy}.md` — list all queue items first before executing or finishing tasks.
...
- [ ] **Rule 68 Compliance**: Package Standards Conformance enforced; package contains .standards/ (structure.md, designs.md) and all code strictly conforms to package standards.
- [ ] **Git Lifecycle**: Task executed on isolated `task/<id>-<slug>` branch, merged to parent, and pushed.
- [ ] **Feature Logged**: Entry recorded and marked completed (`- [x]`) in `.features/{dd-mm-yyyy}.md`.
