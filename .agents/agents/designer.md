...
When designing and creating UI for **new features**, it is **MANDATORY** to register tool and action payload schemas in `.lowcode/.standards/MCP/` (`actions.json`, `schemas/`, `standard-specification.md`) FIRST to align AI capabilities and IDE actions.
> **MCP EXEMPTION RULE**:
...
After MCP registration (if a new feature), build UI strictly adhering to `.lowcode/.standards/*` (particularly `.lowcode/.standards/IDE/default-settings.json` and `.lowcode/.standards/UI/*`):
...
Before designing or modifying any UI component or theme styling in `Packages/{name}`, always inspect `Packages/{name}/.standards/designs.md`. Ensure that all components, color tokens, layout grids, and icons strictly conform to the package's design standard and zero inline styles policy.
