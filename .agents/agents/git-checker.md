...
When receiving an input queue, batch tasks, or multi-item prompt, list ALL items from the queue into `.features/{dd-mm-yyyy}.md` (prioritizing current date) as uncompleted checkboxes `[ ]` (or `[ ] [BUG]`). Only at the last queue item (once all items in the queue are logged in `.features/`), begin executing and finishing the tasks.
