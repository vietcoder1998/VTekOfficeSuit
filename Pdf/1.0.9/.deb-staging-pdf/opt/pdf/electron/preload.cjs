/**
 * packages/Pdf/electron/preload.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK PDF Reader & Studio — Electron Preload Bridge
 *
...
function loadElectronModule() {
  try {
    ...
    console.warn(
      "[Pdf Preload] Electron module not available in current environment:",
...
const electronBridgeApi = {
  ...
  minimizeWindow: () => {
    try {
      ...
      console.error("[Pdf Preload Error] minimizeWindow failed:", minimizeError);
  ...
  maximizeWindow: () => {
    try {
      ...
      console.error("[Pdf Preload Error] maximizeWindow failed:", maximizeError);
      return Promise.resolve(false);
    }
  },
  ...
  closeWindow: () => {
    try {
      ...
      console.error("[Pdf Preload Error] closeWindow failed:", closeError);
      return Promise.resolve(false);
    }
  },
  ...
  isMaximized: () => {
    try {
      ...
      console.error("[Pdf Preload Error] isMaximized failed:", checkMaximizedError);
      return Promise.resolve(false);
    }
  },
  ...
  onWindowStateChange: (callback) => {
    ...
    const listener = (_event, isMaximizedState) => {
      try {
        ...
        console.error("[Pdf Preload Error] onWindowStateChange listener error:", listenerError);
      }
    };
    ...
    return () => {
      try {
        ...
        console.error("[Pdf Preload Error] Failed to remove listener:", removeError);
      }
    };
  },
  ...
  openFileDialog: (options = {}) => {
    try {
      ...
      console.error("[Pdf Preload Error] openFileDialog failed:", openDialogError);
      return Promise.resolve({ canceled: true });
    }
  },
  ...
  saveFileDialog: (options = {}) => {
    try {
      ...
      console.error("[Pdf Preload Error] saveFileDialog failed:", saveDialogError);
      return Promise.resolve({ canceled: true });
    }
  },
  ...
  readFile: (targetFilePath) => {
    try {
      ...
      console.error("[Pdf Preload Error] readFile failed:", readFileError);
  ...
  writeFile: (targetFilePath, fileContentText) => {
    try {
      ...
      console.error("[Pdf Preload Error] writeFile failed:", writeFileError);
  ...
  onExternalFileOpen: (callback) => {
    ...
    const listener = (_event, openedFilePath) => {
      try {
        ...
        console.error("[Pdf Preload Error] onExternalFileOpen listener error:", listenerError);
      }
    };
...
if (contextBridge && typeof contextBridge.exposeInMainWorld === "function") {
  try {
    ...
    console.error("[Pdf Preload Error] Failed to expose electronAPI via contextBridge:", exposeError);
