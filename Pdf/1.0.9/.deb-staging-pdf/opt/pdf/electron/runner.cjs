/**
 * packages/Pdf/electron/runner.cjs
 * ─────────────────────────────────────────────────────────────────────────────
 * 2-TEK PDF Reader & Studio — Electron Development Orchestrator
 *
 * Checks/starts the Next.js dev server on port 3028, waits for HTTP readiness,
...
function resolvePortFromEnv() {
  try {
    ...
    console.error("[Pdf Runner Error] Failed resolving port from env:", resolvePortError);
  }
...
function checkServerReady(targetUrl = DEV_SERVER_URL) {
  try {
    ...
    console.error("[Pdf Runner Error] checkServerReady unexpected failure:", checkError);
    return Promise.resolve(false);
  }
}

async function waitForServer(targetUrl = DEV_SERVER_URL) {
  console.log(`[Pdf Runner] Waiting for dev server at ${targetUrl}...`);
  for (let attemptNumber = 1; attemptNumber <= MAXIMUM_POLL_ATTEMPTS; attemptNumber += 1) {
    ...
    if (isReady) {
      console.log(`[Pdf Runner] Dev server is ready at ${targetUrl} (attempt ${attemptNumber})`);
      return true;
    }
    await new Promise((resolve) => setTimeout(resolve, POLL_INTERVAL_MILLISECONDS));
  }
  console.warn(`[Pdf Runner] Server did not become ready after ${MAXIMUM_POLL_ATTEMPTS} attempts, launching anyway.`);
  return false;
}
...
async function startDevelopmentWorkflow() {
  ...
  if (!isAlreadyRunning) {
    console.log(`[Pdf Runner] Starting Pdf Next.js server on port ${TARGET_PORT}...`);
    ...
    nextServerProcess = spawn(npmCommand, ["run", "dev"], {
      ...
      env: {
        ...
        PDF_PORT: String(TARGET_PORT),
      },
    });

    nextServerProcess.on("error", (spawnError) => {
      console.error("[Pdf Runner] Failed to spawn Next.js dev server:", spawnError);
    });
  } else {
    console.log(`[Pdf Runner] Dev server is already active on port ${TARGET_PORT}`);
  }
  ...
  console.log(`[Pdf Runner] Launching Electron with ${electronExecutableCommand}...`);
  ...
  electronChildProcess.on("exit", (exitCode) => {
    console.log(`[Pdf Runner] Electron exited with code ${exitCode}`);
...
if (require.main === module) {
  startDevelopmentWorkflow().catch((runnerError) => {
    console.error("[Pdf Runner Error] Execution failed:", runnerError);
    process.exit(1);
  });
}
